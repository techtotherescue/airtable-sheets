...
class Sheet{
  ...
  appendRows(records){
    ...
    let data = [];
    for (let record of records)
      // HACK: If a field is of type "object" (anything but the native
      // Javascript types, we store its JSON representation instead of the
      // object itself. This isn't ideal and can lead to bugs:upon loading the
      // data. Specifically, when reading a backup we'll need to try parsing
      // every value as if it were a JSON. If it fails, we know for sure that
      // the original value was a native Javascript type (though we don't know
      // which one), but if it succeeds we don't know whether the original value
      // was an object or a JSON string. If it was a JSON string, we would
      // incorrectly parse it instead of leaving it as a string.
      data.push(header.map(f => (typeof(record[f]) === "object") ? JSON.stringify(record[f]): record[f]));
